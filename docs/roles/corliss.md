...
| `/account/` | the member's own name and email, and the only place either can be changed. **Signed-in, not membership-gated** — an applicant sitting in the queue is exactly the person an admin is about to read a name for. Both fields are seeded from the member's PDS at login and only when blank, so a save here survives their next sign-in; clearing one re-arms the fill. `v1.0.0`+ |
...
| `/manage/` | the cluster console — the application queue, member roll, admin roster, and the reconcile button. Gated on the atproto roster (`is_cluster_admin`), **not** on any Django flag and **never on membership**, so it stays reachable on a rebuilt cluster with an empty membership cache. The queue lists only applications still awaiting a decision (`v0.8.1`+), and since `v0.8.2` an admin approves, changes a tier and revokes from here — authored by their own registry session, never by a Corliss credential. Since `v0.9.0` it also owns roster editing, which was the last thing the `manage_console` SPA was needed for — that role has been deleted, so this is now the cluster's only admin write surface |
| `/systems/` | the stack, and whether it is up — each service asked directly, results cached 30s (10s when anything is not up). Three states: `up` and `down` are measurements, `unknown` is what a blank setting or a probe that raised gets, so a missing address never renders as an outage. Two "up"s are narrower than they read: the **sync relay**'s `/health` is liveness only and never touches Postgres, and **Garage** is asked on its S3 port because its admin API is loopback-only. Cluster-admin gated, 404 otherwise. `v0.9.3`+ |
...
| Clone the app at its pinned ref | `ansible.builtin.git` (`version: {{ corliss_version }}`, `force: true`) | Checks out `Z-Space-Society/Corliss` into `{{ corliss_src }}`. Public repo over the host's NAT — no credentials, nothing in the vault. `force` discards any on-CT drift so the checkout is exactly the pinned tag (the guarantee rsync's `delete: true` used to give). Registered, so collectstatic can clear when it moved. Notifies restart. |
| Probe + install pinned `uv` | `command`, then `get_url`/`unarchive`/`copy` | Install uv reproducibly from the **pinned, checksummed** release tarball (not `curl \| sh`), same idiom as `open-webui`. Skipped when the installed `uv --version` already matches. The probe sets `check_mode: false`, so `--check` reads the real version instead of always simulating an install that then fails at extract. |
...
| `corliss_version` | `v1.2.1` | The **pinned tag** cloned onto the CT. `v1.2.1` fills in `/systems/`' **zai-ops and Version columns** from the manifests every service play writes to Garage since zai-ops v0.7.0 ([ADR-0009](../decisions/0009-service-manifests-in-garage.md)). **Needs the three `Service manifests` keys** (`MANIFEST_BUCKET`, `MANIFEST_ACCESS_KEY`, `MANIFEST_SECRET_KEY`), which carry the read-only manifest key and so sit in their own block apart from the credential-free probe keys. Blank leaves both columns reading "not recorded" and changes nothing else; a service whose play has not replayed on v0.7.0 or later reads the same until it does. A pin below `v1.2.1` ignores the keys. `v1.2.0` adds per-domain themes and a usable phone nav (no new env keys). `v1.0.0` gives members a name. A member was identified by DID, handle and email and nowhere by a name, so the console, the nav and the `id_token` handed to open-webui all showed a handle where a reader expects a person. Corliss now reads `displayName` off the member's own `app.bsky.actor.profile` record at login — unauthenticated, through `com.atproto.repo.getRecord`, so **no new OAuth scope and nobody re-consents** (contrast `v0.8.0`) — and `/account/` lets them change it, and their email, afterwards. **One migration** (`0005_user_display_name`) adds a blank column; the migrate task already runs it before the new code serves. No new env keys. Three things to know before pinning it: **names arrive as people sign in, not at deploy** (there is no backfill, so the console looks unchanged on deploy day and fills in over the following days); **a PDS email change stops propagating** once a member has an email here, because login now only fills a blank one — `/account/` lets them set it, and an edit that reverted at their next sign-in would make that page a lie; and **open-webui picks the name up with no config change**, since the `id_token` gains a `name` claim while `OAUTH_USERNAME_CLAIM=handle` stays as it is (the handle is still what the RP keys the account on). Both fields are display-only — DID is what everything keys on. A pin below `v1.0.0` is the old behaviour with an unused column left behind, which is harmless. `v0.9.3` makes `/systems/` tell the truth: it has listed the stack since `v0.7.2` and reported "unknown" for everything but itself and Postgres, and now asks each service directly and reports up, down or unknown. **Needs the four `Health probes` keys** — `SYNC_RELAY_URL`, `REDIS_URL`, `GARAGE_S3_URL`, `CADDY_HEALTH_URL` — the only services corliss had no address for; HappyView, LiteLLM and Open WebUI reuse the ones it already calls them on. Two consequences: **this role now fails if `sync-relay`, `redis`, `object-store` or `proxy` has no CTID assigned** (the URLs derive from `hostvars` unguarded, same idiom as `corliss_membership_registry_url`; `zai-assign <service> <ctid>` is the fix, and it is an inventory dependency, not an ordering one), and it **drops the page's "Manage Console" row**, which described static files already deleted with the `manage_console` role — the same deletion behind `corliss_manage_url` being blank. Blank probe settings mean "unknown", never "down". A pin below `v0.9.3` ignores the four keys and the page reverts to reporting "unknown"; nothing fails. `v0.9.2` adds the other half of a decision — an application can be **declined** from the queue, where until now the only way to answer "no" was to leave the row sitting in it. **Nothing is deployed at the registry for this**: a decline writes an ordinary revocation, and with no grant before it that is already what "not a member" resolves to under latest-event-wins. It survives a reconcile because the revocation is an admin-authored event, so the DID is accounted for rather than orphaned — load-bearing, since the queue reads "already decided" off that cache row and a rebuild that dropped it would put every declined applicant back. The record carries `reason: "Application declined."` so the log does not read as revoking a membership that never existed, and Decline **refuses when the subject is a current member** (the queue can hold one, flagged "asked again", and there it would silently revoke a sitting member and dismiss them as an admin). Applicants' handles link out to their `bsky.app` profile, addressed by DID since a handle is mutable. No new env keys, no migration; a pin below `v0.9.2` has no decline button. `v0.9.1` is the membership console's presentation only — no new env keys, no migration, nothing to deploy in scn-member-registry or happyview. The members table stopped scrolling sideways: every per-member write (tier, revoke, appoint/retire admin) left the `Change` column for a panel opened by clicking the member's handle, which also carries their DID and who granted them. That panel is pure CSS `:target` and **uses no JavaScript**, deliberately — `/manage/` holds the reconcile button and is how a Corliss rebuilt from nothing gets its membership back, so its controls must not depend on a script loading. Members now sits above Applications (the queue is usually empty, and the page opened on "Nobody has applied"), the service-session lock is a drawn icon reading "Admin Locked"/"Admin Unlocked" instead of an emoji pair that differed only by the tilt of a shackle, and page titles read "— SCN". A pin below `v0.9.1` is the old layout, identical in behaviour. `v0.9.0` brings admin roster editing into `/manage/` — the last thing the `manage_console` SPA was still needed for, and what made deleting that role possible. Appointing an admin does **both halves**: the roster entry in the service DID's repo and registry space write access via `setSpaceAccess`, because space membership is what makes a grant authoritative at HappyView, so a roster entry alone reaches the console but cannot approve anyone. Retiring stamps `removedAt` (never deletes) and removes space access. **The service account must be signed in** — the roster lives in its repo, so `/manage/unlock` authenticates it through the ordinary callback route (no second redirect URI, no re-consent for members; its login is the only one asking a wider atproto scope). The space-access half is **reported, never raised**: a failure leaves the roster write standing and asks for the same action again, which both halves are idempotent under. `make_admin` no longer grants Django `is_superuser` alongside roster authority. No new env keys, no migration; a pin below `v0.9.0` leaves roster editing in the Manage Console. `v0.8.3` fixes the DPoP proof that made every approve in `v0.8.2` fail with 401 `htu mismatch` — pin `v0.8.2` and the console shows the buttons and cannot use them. `v0.8.2` adds approve, tier change and revoke to `/manage/`, which **requires `MEMBERSHIP_REGISTRY_CLIENT_KEY`** — reads never did, but a write is a DPoP-authenticated procedure call made as the approving admin, and no shared token can produce that proof. Corliss obtains the session by having the registry provision a DPoP key during an *admin's* login; **admins must sign out and in again** after this deploys, a registry outage degrades to "no approvals" rather than "no login", and one migration adds a nullable timestamp. `v0.8.1` narrows the `/manage/` application queue to what is still awaiting a decision, and shows a signed-in non-member the Chat and API nav entries as disabled rather than as a live link the gate would refuse. `v0.8.0` makes the apply button work — it writes the application into the applicant's own PDS, needing no new env key and no migration, and **narrows the OAuth scope** from `transition:generic` to `repo:network.sharedcomputer.membership.request`, so every existing member re-consents on their next sign-in (old tokens still read; a member applying before re-authorising is told to sign in again). A pin below `v0.8.0` leaves the button inert, as it was. `v0.7.0` issues members' LiteLLM API keys from `/api/` and needs the three `LITELLM_*` keys in `corliss.env.j2`; a pin below it ignores them, and a pin at or above it with them unset leaves `/api/` saying it is not configured. `v0.6.1`/`v0.6.0` end the relying party's session on revocation (needs `OIDC_BACKCHANNEL_LOGOUT_URI` and the `redis` CT); `v0.5.0` closes GATE; `v0.4.1` presents the registry's public Host on the internal call, without which every reconcile fails 421; `v0.4.0` reconciles membership and serves `/manage/`, needing the `MEMBERSHIP_REGISTRY_*` keys; `v0.3.2` adds the home page, `/api/` and the nav's identity/admin menu, and reads the admin roster from `SCN_SERVICE_DID`; `v0.3.0` added atproto membership caching. Bumping this is how the app is upgraded — and since `v0.2.1` the deployed site's footer stamps the tag it's running (resolved by `git describe --tags` off the checkout), so the pin is verifiable from a browser. Floor is `v0.2.0`, the uv-project release (pyproject + `uv.lock`, Python 3.14, Django 6.1) this role's `uv sync --locked` requires. |
...
| `corliss_manage_url` | `""` | Outbound link to a *separate* admin console, offered on `/manage/` as a fallback for an admin whose sign-in picked up no registry session. **Blank on purpose, unlike the other links here**: the SPA it pointed at was deleted with the `manage_console` role, so `manage.<domain>` resolves to nothing and a derived value would render a live-looking link to a dead origin. Corliss treats blank as "no console" and drops the offer; the knob remains for a cluster that stands a second admin surface up elsewhere. |
...
| `corliss_sync_relay_url` | `http://{{ hostvars['sync-relay'].ansible_host }}:7030` | What `/systems/` dials to check the [sync relay](sync_relay.md). Its `/health` is **liveness only** and deliberately never touches Postgres, so "up" means the process is serving, not that its storage works. Since `v1.2.1` the row no longer says so, so read a green dot there with that in mind. |
| `corliss_sync_relay_port` | `7030` | Local mirror of `sync_relay_port`, same convention as `corliss_openwebui_port`. Keep the two in sync. |
| `corliss_redis_url` | `redis://{{ hostvars['redis'].ansible_host }}:6379` | What `/systems/` dials to check [redis](redis.md). Corliss holds **no redis client and no redis dependency** — the probe writes `PING` to a socket. `requirepass` is set, so the answer is `-NOAUTH`, and that refusal proves liveness exactly as `+PONG` would: only a Redis that is up and speaking its protocol can produce it. No password is passed and none is needed. Not a cache backend — corliss's cache is LocMemCache, and pointing it here would be a separate, deliberate change. |
| `corliss_redis_port` | `6379` | Local mirror of `redis_port`. |
| `corliss_garage_s3_url` | `http://{{ hostvars['object-store'].ansible_host }}:3900` | What `/systems/` dials to check [Garage](object_store.md). Its **S3 port, not its admin API** — `garage.toml` binds the admin API to `127.0.0.1:3903`, unreachable from the corliss CT. An unauthenticated `GET /` on an S3 endpoint answers an XML error, so corliss counts *any* HTTP status as up: a 403 from Garage is a serving Garage. |
| `corliss_garage_s3_port` | `3900` | Local mirror of Garage's S3 API port. |
...
same place, and is set as `RECONCILE_TOKEN` in scn-member-registry's `.env` before
`npm run deploy`:
...
- **A CTID assigned for [`sync-relay`](sync_relay.md), [`redis`](redis.md),
  ...
  this role's template rather than rendering a blank that looks configured. Same
  idiom and same reasoning as `corliss_membership_registry_url`. Note this is an
  *inventory* dependency, not an ordering one: nothing here reads a fact from
  those hosts, so their plays need not have run. `zai-assign <service> <ctid>` is
  the fix if a run fails on one.
...
- **Two variable names outlive the console they were named for, deliberately.**
  `console_client_key` and `SCN_SERVICE_DID` were shared with the
  `manage_console` role until it was deleted; corliss is the only consumer now,
  and both were rehomed into this role's defaults rather than renamed. The
  rename is a *runtime-inventory migration*, not an edit: the live cluster's
  git-ignored `inventory/local.yml` already holds `console_client_key`, so
  changing the name here without a migration silently falls back to the blank
  default — which for the client key means reconciliation quietly loses its
  origin-bound key rather than failing loudly. `console_client_key` →
