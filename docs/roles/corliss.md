...
| `/manage/` | the cluster console — the application queue, member roll, admin roster, and the reconcile button. Gated on the atproto roster (`is_cluster_admin`), **not** on any Django flag and **never on membership**, so it stays reachable on a rebuilt cluster with an empty membership cache. The queue lists only applications still awaiting a decision (`v0.8.1`+), and since `v0.8.2` an admin approves, changes a tier and revokes from here — authored by their own registry session, never by a Corliss credential. Supersedes [`manage_console`](manage_console.md) for everything but roster editing |
